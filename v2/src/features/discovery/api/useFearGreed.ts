...
/** Crypto Fear & Greed Index. Numeric fields arrive as strings; errors are in-band. */
export function useFearGreed() {
  return useQuery({
    queryKey: queryKeys.fearGreed(),
    queryFn: ({ signal }) => jsonFetch<FngResponse>(FNG_URL, signal),
    staleTime: 60 * 60_000, // updates ~daily
    select: (res) => {
      if (res.metadata.error) throw new Error(res.metadata.error);
      const latest = res.data[0];
      // history is newest-first; reverse for a left-to-right sparkline
